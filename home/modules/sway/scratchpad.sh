...
show_window_in_position() {
	...
	$wm_msg -q -- "[con_mark=\"$id_mark\"] move position $x_pos px $y_pos px, resize set $width px $height px"
}
...
animate() {
	...
	# Sometimes resize fails initially if the window is still opening,
	# so just resize on every step to ensure minimal screen tearing.
	$wm_msg -q -- "[con_mark=\"$id_mark\"] move position $x px $y px, resize set $width px $height px"
	y=$((y + y_inc))
	x=$((x + x_inc))
